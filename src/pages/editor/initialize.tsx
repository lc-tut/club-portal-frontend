...
import { FormControl, FormErrorMessage, FormLabel, Grid, GridItem, Input, Stack, Textarea, VStack, Radio, RadioGroup, Wrap, HStack, Box, Select, Text, Flex, NumberInput, NumberInputField, NumberInputStepper, NumberIncrementStepper, NumberDecrementStepper, Switch } from "@chakra-ui/react"
import { string, z } from "zod"

import { AxiosRequestConfig } from "axios"
import { EditorBase } from "../../components/common/Editor/EditorBase"
import { TitleArea } from "../../components/global/Header/TitleArea"
import { PADDING_BEFORE_FOOTER, VALID_SNS_LIST, MONTHS, DATE_MAP, TOGGLE_TIME, TOGGLE_DATE, BUILDING_ID_MAP, TOGGLE_ROOM } from "../../utils/consts"
...
import { EditorLabel, EditorText } from "../../components/common/Editor/CommonEditor"
...
const schema = z.object({
    ...
    description: z.string().min(1, { message: "サークルの説明文を入力してください" }),
    shortDescription: z.string().min(1, { message: "サークルの簡易説明文を入力してください" }),
    campus: z.union(
    ...
    contents: z.array(z.object({
        ...
        (contents) => contents.some((item) => item.content.trim() !== ""),
        { message: "活動内容は最低1つ以上入力してください" }
    ),
    links: z.array(z.object({
        label: z.string().min(1, { message: "リンクラベルを設定してください" }),
        url: z.string().url({ message: "正しいURLを入力してください。" }).min(1),
    })).refine(
        (links) => links.some((item) => item.url.trim() !== ""),
    ...
    achievements: z.array(z.object({
        ...
        (achievements) => achievements.some((item) => item.achievement.trim() !== ""),
    ...
    activityDetails: z.array(z.object({
        ...
        (activityDetails) => activityDetails.some((item) => item.place.trim() !== ""),
...
export const InitializeEditor: React.FC<{}> = () => {
    ...
    // スケジュール追加用
    const { fields: scheduleFields, append: scheduleAppend, remove: scheduleRemove } = useFieldArray({
        control,
        name: "schedules"
    })
    ...
    // アチーブメント追加用
    const { fields: achievementFields, append: achievementAppend, remove: achievementRemove } = useFieldArray({
        control,
        name: "achievements"
    })
    ...
    // 活動日時・場所追加用
    const { fields: activityDetailFields, append: activityDetailAppend, remove: activityDetailRemove } = useFieldArray({
        control,
        name: "activityDetails"
    })
    const date_options: Array<EditorSelectOptionItem> = Object.entries(DATE_MAP).map(
        (d) => ({ displayName: d[1], value: d[0] })
    )
    const place_options: Array<EditorSelectOptionItem> = Object.entries(BUILDING_ID_MAP).map(
        (d) => ({ displayName: d[1], value: d[0] })
    )
                            ...
                            <FormControl isInvalid={errors.campus !== undefined}>
                                ...
                                <Controller
                                    ...
                                    render={({ field }) => (
                                        ...
                                        >
                                            <Stack spacing="6rem" direction="row" justify="center" pt="2rem">
                            ...
                            <FormControl isInvalid={errors.clubType !== undefined}>
                                ...
                                <Controller
                                    ...
                                    render={({ field }) => (
                                        ...
                                        >
                                            <Stack spacing="2.5rem" direction="row" justify="center" pt="2rem">
                                    ...
                                    <EditorButton
                                        ...
                                        onClick={() => {
                                            ...
                                            if (linkLabel.trim() && linkUrl.trim()) {
                                                linkAppend({
                                                    label: linkLabel !== "other" ? linkLabel.trim() : customLinkLabel.trim(),
                                                    url: linkUrl.trim()
                                                });
                                                setLinkLabel("");
                                                setLinkUrl("");
                                ...
                                <HStack textColor="text.main">
                                    ...
                                    <Box pl="2.4rem">
                                        <EditorButton
                                            ...
                                            paddingTop="0rem"
                                            onClick={() => {
                                                if (building && (state.isDateDisabled || date) && (state.isTimeDisabled || (startHour && endHour))) {
                                                    activityDetailAppend({
                                                        date: date as DateType,
                                                        timeId: toTimeID(date as DateType, state.isTimeDisabled ? 0 : startHour, state.isTimeDisabled ? 0 : startMinute, state.isTimeDisabled ? 0 : endHour, state.isTimeDisabled ? 0 : endMinute),
                                                        time: state.isTimeDisabled ? "00:00-00:00" : `${startHour}:${startMinute}-${endHour}:${endMinute}`,
                                                        timeRemark: timeRemark,
                                                        placeId: toPlaceID(Number(building), state.isRoomDisabled ? 0 : buildingRoom!),
                                ...
                                <HStack spacing="1.5rem" >
                                    ...
                                    <Stack spacing="0" pt="1.2rem" pb="1.2rem">
                                        ...
                                        >
                                            <NumberInputField
                                                backgroundColor="#fff"
                                                onChange={(e) => setBuildingRoom(Number(e.target.value))}
                                            />
                                            <NumberInputStepper>
                                                <NumberIncrementStepper />
                                                <NumberDecrementStepper />
                                    ...
                                    <HStack key={field.id} alignItems="start" spacing="1rem">
                                        <Box pt="0.5rem" display="flex" alignItems="center">
                                            <Box
                                                ...
                                                flex="1"
                                            >
                                                {!field.date && `${field.date} - `}{!field.time && `${field.time} - `}{field.place}
                                            </Box>
                                            <EditorButton
                                                icon="remove"
                                                paddingTop="0rem"
